...
public class BasicCommands {
	...
	// highlighting summon scope
	public static void drawScope(ActorRef out,GameState gameState,ArrayList<Tile> tiles, int mode){
	...
	public static void unHighlighting(ActorRef out, GameState gameState){
		//Cancel highlighted card
		if(gameState.getCurrentCard() != null && gameState.getMode() == 0){
			BasicCommands.drawCard(out, gameState.getCurrentCard(), gameState.getCurrentCard().getHandPosition(),0);
		}
		//Cancel highlighted Summon scope
		if(gameState.getCurrentTiles() != null){
	...
	/**
	 * Reset the position of all handCards,
	 * If a Card is left empty after it has been used, the empty Card will advance one slot
	 * */
	public static void resetCardsInHands(ActorRef out,GameState gameState,int mode) {
		ArrayList<Card> cardsInHands = new ArrayList<>();
		int position = 0;
		// mode 0: Reset the PlayerCardsInHands
		if(mode == 0){
			cardsInHands = gameState.getP1CardsInHands();
		}
		// mode 1: Reset the AiCardsInHands
		if(mode == 1){
			cardsInHands = gameState.getAiCardsInHands();
		}
		// Redraw all handCards according to the new position
		for(Card card:cardsInHands){
			position = cardsInHands.indexOf(card) + 1;
			BasicCommands.drawCard(out,card,position,0);
			try { Thread.sleep(100); } catch (InterruptedException e) { e.printStackTrace();}
		}
		//Last card should delete
